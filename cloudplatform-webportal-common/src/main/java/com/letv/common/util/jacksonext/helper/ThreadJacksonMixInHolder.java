package com.letv.common.util.jacksonext.helper;
...
/**
 * 当前线程内保存ObjectMapper
 * 供Jackson2HttpMessageConverter使用
 * @author linzhanbo .
 * @since 2016年7月7日, 下午5:41:06 .
 * @version 1.0 .
 */
    ...
    public static ObjectMapper builderMapper() {
        ...
        if (holder.mapper == null && isContainsMixIn()) {
            ...
            for (Map.Entry<Class<?>, Class<?>> mixIn : holder.mixIns) {
                holder.mapper.getSerializationConfig().addMixInAnnotations(mixIn.getKey(), mixIn.getValue());
            }
        }
