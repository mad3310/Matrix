 ...
 * 
 * @author linzhanbo .
 * @since 2016年7月7日, 下午5:41:43 .
 * @version 1.0 .
 */
public class Jackson1JavassistFilterPropertyHandler implements
		FilterPropertyHandler {

	public static final Logger LOGGER = Logger
			.getLogger(Jackson1JavassistFilterPropertyHandler.class);
	...
	@Deprecated
	public Jackson1JavassistFilterPropertyHandler(
			boolean isResponseBodyAnnotation) {
	...
	private void processIgnorePropertiesAnnotation(
			JsonFilterProperties properties,
	...
	public Map<Class<?>, Class<?>> getProxyMixInAnnotation(Method method) {
		...
		ExcludeProperty ignoreProperty = method
				.getAnnotation(ExcludeProperty.class);
		IncludeProperty allowProperty = method
				.getAnnotation(IncludeProperty.class);
	...
	private ObjectMapper createObjectMapper(Map<Class<?>, Class<?>> map) {
		...
		for (Iterator<Entry<Class<?>, Class<?>>> iterator = entries.iterator(); iterator
			...
			mapper.getSerializationConfig().addMixInAnnotations(entry.getKey(),
					entry.getValue());
