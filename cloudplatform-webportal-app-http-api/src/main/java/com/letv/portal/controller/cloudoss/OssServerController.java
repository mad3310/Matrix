...
public class OssServerController {
	...
	private final static Logger logger = LoggerFactory.getLogger(OssServerController.class);
    ...
    public @ResponseBody ResultObject postFile(@PathVariable Long id, @RequestParam MultipartFile file,String directory){
        isAuthoritySwift(id);
        String fileUrl = this.swiftServerProxy.postFiles(id, file, directory);
        ResultObject obj = new ResultObject();
        obj.setData(fileUrl);
        return obj;
    }
    ...
    private void isAuthoritySwift(Long id) {
        if(id == null)
            throw new ValidateException("OSS服务id无效");
