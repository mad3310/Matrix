...
function queryByPage() {
	...
	$.ajax({
		cache:false,
		type : "get",
		url : queryUrlBuilder("/es/cluster",queryCondition),
		success : function(data) {
			...
			for (var i = 0, len = array.length; i < len; i++) {
				...
				var esId ="<td class='center'>"
								...
								+"</label>"
							+"</td>";
				var esClusterName="<td>"
						+  "<a class='link' href='/detail/cluster/" + tempObj.id+"'>"+tempObj.clusterName+"</a>"
						+"</td>";
				if(tempObj.hcluster){
					var esHclusterNameAlias="<td class='hidden-480'>"
